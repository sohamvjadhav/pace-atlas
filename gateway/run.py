...
# Resolve PACE home directory (respects PACE_HOME override)
from hermes_constants import get_hermes_home
from utils import atomic_yaml_write
_hermes_home = get_hermes_home()

# Load environment variables from ~/.pace/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from pace_cli.env_loader import load_hermes_dotenv
...
if _config_path.exists():
    try:
        ...
        from pace_cli.config import _expand_env_vars
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from pace_cli.runtime_provider import (
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `pace skills config`"
                    )
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `pace skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.pace/config.yaml, returning {} on any error."""
    try:
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    """Resolve the PACE update command as argv parts.

    Tries in order:
    1. ``shutil.which("pace")`` — standard PATH lookup
    2. ``sys.executable -m pace_cli.main`` — fallback when PACE is running
       from a venv/module invocation and the ``pace`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("pace_cli") is not None:
            return [sys.executable, "-m", "pace_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the pace-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("pace-agent-setup") is not None
        except Exception:
            return False
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        the prefill_messages_file key in ~/.pace/config.yaml.
        Relative paths are resolved from ~/.pace/.
        """
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        agent.system_prompt in ~/.pace/config.yaml.
        """
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting PACE Gateway...")
        logger.info("Session storage: %s", self.config.sessions_dir)
        try:
            from pace_cli.profiles import get_active_profile_name
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.pace/.env to allow open access, "
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'pace-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`pace pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _quick_key in self._running_agents:
            ...
            from pace_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in pace_cli/commands.py — no hardcoded set to maintain here.
        from pace_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                from pace_cli.plugins import get_plugin_command_handler
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where PACE delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the PACE venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill pace-agent-setup`"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from PACE_HOME path
        # Profile paths look like: ~/.pace/profiles/<name>
        profiles_parent = Path.home() / ".pace" / "profiles"
        try:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from pace_cli.commands import gateway_help_lines
        lines = [
            "📖 **PACE Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        """
        import yaml
        from pace_cli.model_switch import (
            switch_model as _switch_model, parse_model_flags,
            list_authenticated_providers,
        )
        from pace_cli.providers import get_label
        ...
        if persist_global:
            try:
                ...
                from pace_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        if current_provider == "auto":
            try:
                from pace_cli.auth import resolve_provider as _resolve_provider
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.pace/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall PACE with the messaging extra, e.g. "
                    "`pip install pace-agent[messaging]`."
                )
    ...
    async def _run_background_task(
        ...
        try:
            ...
            from pace_cli.tools_config import _get_platform_tools
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update PACE Agent to the latest version.

        Spawns ``pace update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``pace update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from pace_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `pace update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update PACE Agent')}"
        ...
        if not hermes_cmd:
            return (
                "✗ Could not locate the `pace` command. "
                "PACE is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `pace update` manually in your terminal."
            )
        ...
        # Spawn `pace update --gateway` detached so it survives gateway restart.
        ...
        self._schedule_update_notification_watch()
        return "⚡ Starting PACE update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``pace update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ PACE update finished.")
                    else:
                        await adapter.send(chat_id, "❌ PACE update failed (exit code {}).".format(exit_code))
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ PACE update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ PACE update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ PACE update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ PACE update finished successfully."
                    else:
                        msg = "❌ PACE update failed. Check the gateway logs or run `pace update` manually for details."
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called pace-agent-setup that can help "
                    "users configure PACE features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called pace-agent-setup "
                                "that can help users configure PACE features "
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `pace cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same PACE_HOME.
    # The PID file is scoped to PACE_HOME, so future multi-profile
    # setups (each profile using a distinct PACE_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            logger.error(
                "Another gateway instance is already running (PID %d, PACE_HOME=%s). "
                "Use 'pace gateway restart' to replace it, or 'pace gateway stop' first.",
                existing_pid, hermes_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'pace gateway restart' to replace it,\n"
                f"   or 'pace gateway stop' to kill it first.\n"
                f"   Or use 'pace gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PACE Gateway - Multi-platform messaging")
