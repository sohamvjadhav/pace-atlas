"""
Gateway subcommand for pace CLI.

Handles: pace gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from pace_cli.config import get_env_value, get_hermes_home, save_env_value, is_managed, managed_error
# display_hermes_home is imported lazily at call sites to avoid ImportError
# when hermes_constants is cached from a pre-update version during `pace update`.
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "pace_cli.main gateway",
        "pace_cli/main.py gateway",
        "pace gateway",
        "gateway/run.py",
    ]
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PACE_HOME-scoped).
...
_SERVICE_BASE = "pace-gateway"
SERVICE_DESCRIPTION = "PACE Atlas Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PACE_HOME.

    Returns ``""`` for the default ``~/.pace``, the profile name for
    ``~/.pace/profiles/<name>``, or a short hash for any other custom
    PACE_HOME path.
    """
    import hashlib
    import re
    ...
    default = (_Path.home() / ".pace").resolve()
    if home == default:
        return ""
    # Detect ~/.pace/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PACE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this PACE_HOME.

    Default ``~/.pace`` returns ``pace-gateway`` (backward compatible).
    Profile ``~/.pace/profiles/coder`` returns ``pace-gateway-coder``.
    Any other PACE_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    pace gateway uninstall")
    print_info("    sudo pace gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so PACE can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo pace gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo pace gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo pace gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.pace`` → ``ai.pace.gateway.plist`` (backward compatible).
    Profile ``~/.pace/profiles/coder`` → ``ai.pace.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.pace.gateway-{suffix}" if suffix else "ai.pace.gateway"
...
def get_pace_cli_path() -> str:
    """Get the path to the pace CLI."""
    # Check if installed via pip
    import shutil
    hermes_bin = shutil.which("pace")
    if hermes_bin:
        return hermes_bin
    
    # Fallback to direct module execution
    return f"{get_python_path()} -m pace_cli.main"
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current PACE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.pace                    → /home/alice/.pace
      /root/.pace/profiles/coder     → /home/alice/.pace/profiles/coder
      /opt/custom-pace               → /opt/custom-pace  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".pace").resolve()
    target_default = Path(target_home_dir) / ".pace"

    # Default ~/.pace → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.pace → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.pace) — keep as-is
...
User={username}
Group={group_name}
ExecStart={python_path} -m pace_cli.main gateway run --replace
...
Environment="PACE_HOME={hermes_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current PACE install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}pace gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}pace gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}pace gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}pace gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}pace gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.pace.gateway-{suffix}" if suffix else "ai.pace.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current PACE install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  pace gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current PACE install")
    else:
        print("⚠ Service definition is stale relative to the current PACE install")
        print("  Run: pace gateway start")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and PACE will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @pace:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. pace) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where PACE delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your PACE agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address PACE will use (e.g., pace@gmail.com)."},
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'pace pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: pace pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'pace gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from pace_cli.main import cmd_whatsapp
    import argparse
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: pace gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: pace gateway")
    else:
        print()
        print_info("No platforms configured. Run 'pace gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: pace gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: pace gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  pace gateway install")
                print("  sudo pace gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  pace gateway          # Run in foreground")
                print("  pace gateway install  # Install as user service")
                print("  sudo pace gateway install --system  # Install as boot-time system service")
