"""
Multi-provider authentication system for PACE Atlas.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.pace/auth.json with cross-process file locking.
...
from pace_cli.config import get_hermes_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "pace-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from pace_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `pace model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.pace/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `pace logout`.
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'pace model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.pace/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.pace/auth.json (not ~/.codex/)
#
# PACE maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from PACE auth store (~/.pace/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `pace login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `pace login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `pace login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `pace login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to PACE auth store (~/.pace/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating PACE auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to PACE auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from PACE's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to PACE auth store")
            print("⚠️  Migrating Codex credentials to PACE's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `pace login` to create a fully independent session.\n")
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other PACE processes
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip PACE models — they're not reliable for agentic tool-calling
            if "pace" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "PACE is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("PACE is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `pace status` output."""
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that PACE can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- PACE provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `pace login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from pace_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'pace model' or 'pace setup' instead."""
    print("The 'pace login' command has been removed.")
    print("Use 'pace model' to select a provider and model,")
    print("or 'pace setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.pace/auth.json."""

    # Check for existing PACE-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in PACE auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("PACE will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            ...
            print("PACE will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — PACE gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(PACE creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to PACE auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting PACE login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("PACE will use OpenRouter for inference.")
        else:
            print("Run `pace model` or configure an API key to use PACE.")
    else:
