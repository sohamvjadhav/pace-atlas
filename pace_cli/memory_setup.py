"""pace memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as pace tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        ...
        print(f"  Then re-run: pace memory setup")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.pace/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("PACE_HOME", os.path.expanduser("~/.pace"))) / ".env"
    env_writes = {}
    ...
    hermes_home = str(Path(os.environ.get("PACE_HOME", os.path.expanduser("~/.pace"))))
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.pace/plugins/")
