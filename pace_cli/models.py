...
Add, remove, or reorder entries here — both `pace setup` and
`pace` provider-selection will pick up the change automatically.
"""
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from pace_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to PACE' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``pace_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from pace_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from pace_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from pace_cli.copilot_auth import copilot_request_headers
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"PACE will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
