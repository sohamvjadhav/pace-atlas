"""
MCP Server Management CLI — ``pace mcp`` subcommand.

Implements ``pace mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.pace/config.yaml under the ``mcp_servers`` key.
"""
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  pace mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  pace mcp add github --command npx --args @modelcontextprotocol/server-github')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: pace mcp test " + name)
        return
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  pace mcp add <name> --url <endpoint>')
        _info('  pace mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'pace mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``pace mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("pace mcp serve                              Run as MCP server")
        _info("pace mcp add <name> --url <endpoint>        Add an MCP server")
        _info("pace mcp add <name> --command <cmd>         Add a stdio server")
        _info("pace mcp remove <name>                      Remove a server")
        _info("pace mcp list                               List servers")
        _info("pace mcp test <name>                        Test connection")
        _info("pace mcp configure <name>                   Toggle tools")
        print()
