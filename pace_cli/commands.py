"""Slash command definitions and autocomplete for the PACE CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update PACE Atlas to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("PACE_HOME", os.path.expanduser("~/.pace")),
            "config.yaml",
        )
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``pace skills
    ...
    try:
        from pace_cli.plugins import get_plugin_manager
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /pace handler.

    Maps both canonical names and aliases so /pace bg do stuff works
    the same as /pace background do stuff.
    """
    overrides = _resolve_config_gates()
