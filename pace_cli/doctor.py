"""
Doctor command for pace CLI.

Diagnoses issues with PACE Atlas setup.
"""
...
from pace_cli.config import get_project_root, get_hermes_home, get_env_path
from hermes_constants import display_hermes_home

PROJECT_ROOT = get_project_root()
PACE_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.pace or ~/.pace/profiles/coder)

# Load environment variables from ~/.pace/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.pace/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `pace`.
    ...
    print(color("│                 🩺 PACE Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.pace/.env (primary location for user config)
    env_path = PACE_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'pace setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'pace setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'pace setup' to create one")
                issues.append("Run 'pace setup' to create .env")
    
    # Check ~/.pace/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = PACE_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from pace_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'pace setup' to migrate config")
                else:
                    issues.append("Run 'pace doctor --fix' or 'pace setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'pace doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from pace_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give PACE a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# PACE Atlas Persona\n\n"
                "<!-- Edit this file to customize how PACE communicates. -->\n\n"
                "You are PACE, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'pace doctor --fix' to checkpoint")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/pace-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'pace setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: pace skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: pace memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: pace memory setup")
            issues.append("No Honcho API key — run 'pace memory setup'")
        else:
    ...
    try:
        ...
        if mem0_key:
            ...
            check_warn("Mem0 not configured", "(set MEM0_API_KEY in .env or run pace memory setup)")
    except ImportError:
    ...
    # =========================================================================
    try:
        from pace_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "pace -p" in content:
                            _m = _re.search(r"pace -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'pace doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
