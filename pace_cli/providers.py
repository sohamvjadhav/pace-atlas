"""
Single source of truth for provider identity in PACE Atlas.
...
2. **PACE overlays** — transport type, auth patterns, aggregator flags,
...
# -- PACE overlay ----------------------------------------------------------
# PACE-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """PACE-specific provider metadata layered on top of models.dev."""
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "pace", "user-config"
...
def get_overlay(provider_id: str) -> Optional[HermesOverlay]:
    """Get PACE overlay for a provider, if one exists."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. PACE overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + PACE overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + pace extra
    ...
    if overlay is not None:
        # PACE-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy PACE IDs (point to same providers)
