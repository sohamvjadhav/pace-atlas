"""
Unified tool configuration for PACE Atlas.

`pace tools` and `pace setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.pace/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from pace_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "pace-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "pace-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "pace-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "pace-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "pace-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "pace-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "pace-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "pace-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "pace-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "pace-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "pace-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "pace-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "pace-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "pace-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "pace-webhook"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/pace-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in pace-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "pace-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "pace-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `pace tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by pace tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (pace-cli, pace-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.pace/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `pace tools` and `pace setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚡ PACE Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(color("  Changes take effect on next 'pace' or gateway restart.", Colors.DIM))
    print()
