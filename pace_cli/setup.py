"""
Interactive setup wizard for PACE Atlas.
...
Config files are stored in ~/.pace/ for easy access.
"""
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from pace_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from pace_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from pace_cli.config import get_env_value
...
# display_hermes_home imported lazily at call sites (stale-module safety during pace update)

from pace_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚡ PACE Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure PACE using environment variables or config commands:")
    print_info("  pace config set model.provider custom")
    print_info("  pace config set model.base_url http://localhost:8080/v1")
    print_info("  pace config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'pace setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from pace_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'pace setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'pace setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'pace setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'pace setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'pace setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('pace setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('pace setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('pace setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('pace setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('pace setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('pace config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('pace config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('pace config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('pace', Colors.GREEN)}              Start chatting")
    print(f"   {color('pace gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('pace doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `pace tools` and `pace setup tools`).
...
def setup_model_provider(config: dict):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``pace model``)
    ...
    provider added to ``pace model`` is automatically available here.
    """
    ...
    # Delegate to the shared pace model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from pace_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: pace model")
    ...
    if _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from pace_cli.auth_commands import auth_add_command
            ...
            print_info(
                "PACE can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend PACE can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one PACE can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'pace setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.pace/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'pace setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where PACE runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using PACE via Telegram/Discord, this is where")
        print_info(
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with PACE from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where PACE delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://pace-agent.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where PACE delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where PACE delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'pace whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'pace whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://pace-agent.nousresearch.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info(
            "   https://pace-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  pace config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     pace config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: pace gateway install")
            else:
                print_info("  You can install later: pace gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo pace gateway install --system")
                print_info("  Or run in foreground:  pace gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   pace gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `pace setup tools` and `pace tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from pace_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from pace_cli.auth import get_active_provider
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("PACE can import your settings, memories, skills, and API keys.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in PACE.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      pace setup           — full or quick (auto-detected)
      pace setup model     — just model/provider
      pace setup terminal  — just terminal backend
      pace setup gateway   — just messaging platforms
      pace setup tools     — just tool configuration
      pace setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚡ PACE Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚡ PACE Atlas Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your PACE Atlas installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have PACE configured.")
        print()
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'pace setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'pace setup' again when ready.")
            return
    ...
    print()
    print_info("You can edit these files directly or use 'pace config edit'")
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'pace setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect PACE to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'pace setup gateway'.")
