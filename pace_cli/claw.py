"""pace claw — OpenClaw migration commands.

Usage:
    pace claw migrate              # Interactive migration from ~/.openclaw
    pace claw migrate --dry-run    # Preview what would be migrated
    pace claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    pace claw cleanup              # Archive leftover OpenClaw directories
    pace claw cleanup --dry-run    # Preview what would be archived
"""
...
from pace_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: pace claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to PACE")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'pace claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → PACE migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: pace claw migrate --source /path/to/.openclaw")
        return
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: pace claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  pace claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  pace claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  pace config set OPENROUTER_API_KEY sk-or-v1-...")
