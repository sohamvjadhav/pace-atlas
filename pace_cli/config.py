"""
Configuration management for PACE Atlas.

Config files are stored in ~/.pace/ for easy access:
- ~/.pace/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.pace/.env         - API keys and secrets

This module provides:
- pace config          - Show current configuration
- pace config edit     - Open config in editor
- pace config set      - Set a specific value
- pace config wizard   - Re-run setup wizard
"""
...
from pace_cli.default_soul import DEFAULT_SOUL_MD
...
def is_managed() -> bool:
    """Check if PACE is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in PACE_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade pace-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "pace update"


def format_managed_message(action: str = "modify this PACE installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this PACE installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.pace-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this PACE installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade pace-agent"
        )

    return (
        f"Cannot {action}: this PACE installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall PACE."
    )
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into PACE_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.pace directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when PACE
    ...
    "browser": {
        ...
        "camofox": {
            # When true, PACE sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.pace/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for pace-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚡ *PACE Atlas*" header.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the PACE auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @pace:example.org)",
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: pace config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.pace/config.yaml."""
    import copy
    ensure_hermes_home()
...
#   openai-codex (OAuth — pace login) — OpenAI Codex
#   nous         (OAuth — pace login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.pace/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.pace/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real PACE env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.pace/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.pace/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.pace/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚡ PACE Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  pace config edit     # Edit config file", Colors.DIM))
    print(color("  pace config set <key> <value>", Colors.DIM))
    print(color("  pace setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: pace config set <key> <value>")
            print()
            print("Examples:")
            print("  pace config set model anthropic/claude-sonnet-4")
            print("  pace config set terminal.backend docker")
            print("  pace config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'pace config migrate' to add them")
        ...
        print("  pace config           Show current configuration")
        print("  pace config edit      Open config in editor")
        print("  pace config set <key> <value>   Set a config value")
        print("  pace config check     Check for missing/outdated config")
        print("  pace config migrate   Update config with new options")
        print("  pace config path      Show config file path")
        print("  pace config env-path  Show .env file path")
        sys.exit(1)
