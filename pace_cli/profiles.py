"""
Profile management for multiple isolated PACE instances.

Each profile is a fully independent PACE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.pace/profiles/<name>/`` by default.

The "default" profile is ``~/.pace`` itself — backward compatible,
zero migration needed.

Usage::

    pace profile create coder          # fresh profile + bundled skills
    pace profile create coder --clone  # also copy config, .env, SOUL.md
    pace profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    pace -p coder chat                 # or via flag
    pace profile use coder             # set as sticky default
    pace profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.pace) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "pace-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "pace", "default", "test", "tmp", "root", "sudo",
})

# PACE subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.pace/profiles/`` — anchored to the user's home,
    NOT to the current PACE_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".pace" / "profiles"


def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) PACE_HOME path."""
    return Path.home() / ".pace"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its PACE_HOME directory."""
    if name == "default":
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, pace subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a pace subcommand"
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.pace)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches PACE_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "PACE_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.pace).\n"
            "To remove everything, use: pace uninstall"
        )
    ...
    # Check for service
    from pace_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PACE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("PACE_HOME")
    try:
        os.environ["PACE_HOME"] = str(profile_dir)
        from pace_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["PACE_HOME"] = old_home
        elif "PACE_HOME" in os.environ:
            del os.environ["PACE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.pace/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: pace profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PACE_HOME.

    Returns ``"default"`` if PACE_HOME is not set or points to ``~/.pace``.
    Returns the profile name if PACE_HOME points into ``~/.pace/profiles/<name>``.
    Returns ``"custom"`` if PACE_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.pace itself — its parent is ~/ and its
        # directory name is ".pace", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: pace profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.pace itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.pace). "
            "Specify a different name: pace profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for pace profile names."""
    return '''# PACE Atlas profile completion
# Add to ~/.bashrc: eval "$(pace completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.pace/profiles"
...
complete -F _hermes_completion pace
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for pace profile names."""
    return '''#compdef pace
# PACE Atlas profile completion
# Add to ~/.zshrc: eval "$(pace completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.pace/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.pace/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PACE_HOME path string.

    Called early in the CLI entry point, before any pace modules
    are imported, to set the PACE_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: pace profile create {profile_name}"
        )
