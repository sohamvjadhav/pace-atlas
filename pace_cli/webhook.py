"""pace webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.pace/webhook_subscriptions.json and are
...
def _hermes_home() -> Path:
    return Path(
        os.getenv("PACE_HOME", str(Path.home() / ".pace"))
    ).expanduser()
...
def _setup_hint() -> str:
  ...
  Then start the gateway: pace gateway run
"""
...
def webhook_command(args):
    """Entry point for 'pace webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: pace webhook {subscribe|list|remove|test}")
        print("Run 'pace webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (pace gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: pace webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from pace webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (pace gateway run)")
