...
- ``pace_cli.providers``        -- canonical provider identity + overlays
- ``pace_cli.model_normalize``  -- per-provider name formatting
...
from pace_cli.providers import (
    ALIASES,
    LABELS,
...
from pace_cli.model_normalize import (
    detect_vendor,
...
def switch_model(
    ...
    from pace_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            return ModelSwitchResult(
                ...
                error_message=(
                    ...
                    f"Check 'pace model' for available providers, or define it "
        ...
        if not new_model:
            if pdef.base_url:
                from pace_cli.runtime_provider import _auto_detect_local_model
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from pace_cli/models.py (OPENROUTER_MODELS,
    ...
    from pace_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by pace provider ID
    ...
    # --- 2. Check PACE-only providers (nous, openai-codex, copilot) ---
    from pace_cli.providers import HERMES_OVERLAYS
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external", "external_process"):
            ...
            try:
                from pace_cli.auth import _read_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: pace setup -> Custom OpenAI-compatible endpoint"
            ),
        )
