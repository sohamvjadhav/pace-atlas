#!/usr/bin/env python3
"""
PACE CLI - Main entry point.

Usage:
    pace                     # Interactive chat (default)
    pace chat                # Interactive chat
    pace gateway             # Run gateway in foreground
    pace gateway start       # Start gateway as service
    pace gateway stop        # Stop gateway service
    pace gateway status      # Show gateway status
    pace gateway install     # Install gateway service
    pace gateway uninstall   # Uninstall gateway service
    pace setup               # Interactive setup wizard
    pace logout              # Clear stored authentication
    pace status              # Show status of all components
    pace cron                # Manage cron jobs
    pace cron list           # List cron jobs
    pace cron status         # Check if cron scheduler is running
    pace doctor              # Check configuration and dependencies
    pace honcho setup                    # Configure Honcho AI memory integration
    pace honcho status                   # Show Honcho config and connection status
    pace honcho sessions                 # List directory → session name mappings
    pace honcho map <name>               # Map current directory to a session name
    pace honcho peer                     # Show peer names and dialectic settings
    pace honcho peer --user NAME         # Set user peer name
    pace honcho peer --ai NAME           # Set AI peer name
    pace honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    pace honcho mode                     # Show current memory mode
    pace honcho mode [hybrid|honcho|local]  # Set memory mode
    pace honcho tokens                   # Show token budget settings
    pace honcho tokens --context N       # Set session.context() token cap
    pace honcho tokens --dialectic N     # Set dialectic result char cap
    pace honcho identity                 # Show AI peer identity representation
    pace honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    pace honcho migrate                  # Step-by-step migration guide: OpenClaw native → PACE + Honcho
    pace version             Show version
    pace update              Update to latest version
    pace uninstall           Uninstall PACE Atlas
    pace acp                 Run as an ACP server for editor integration
    pace sessions browse     Interactive session picker with search

    pace claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (pace tools, pace setup, pace model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'pace {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any pace module import.
#
# Many modules cache PACE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("PACE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.pace/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PACE_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.pace/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".pace" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set PACE_HOME
    if profile_name is not None:
        try:
            from pace_cli.profiles import resolve_profile_env

            pace_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent pace from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["PACE_HOME"] = pace_home
...
# Load .env from ~/.pace/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from pace_cli.config import get_pace_home
from pace_cli.env_loader import load_pace_dotenv

load_pace_dotenv(project_env=PROJECT_ROOT / ".env")
...
from pace_cli import __version__, __release_date__
from pace_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from pace_cli.config import get_env_path, get_pace_home, load_config
    from pace_cli.auth import get_auth_status

    # Determine whether PACE itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from pace_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_pace_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from pace_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_pace_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if PACE has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants PACE to use their tokens.
    if _has_pace_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from pace_state import SessionDB
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'pace sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like PACE isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  pace setup")
        print()

        from pace_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'pace setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from pace_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from pace_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from pace_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with PACE?")
        print()
    ...
    session_dir = get_pace_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: pace gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  pace gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚡ PACE Atlas'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  pace gateway")
        ...
        print()
        print("  Or install as a service: pace gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'pace whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from pace_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``pace model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from pace_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from pace_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from pace_cli.models import model_ids
    ...
    if selected:
        ...
        from pace_cli.config import load_config, save_config
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from pace_cli.config import get_env_value, save_config, save_env_value
    from pace_cli.nous_subscription import (
    ...
    from pace_cli.models import _PROVIDER_MODELS
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from pace_cli.codex_models import get_codex_model_ids
    from pace_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from pace_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from pace_cli.auth import _save_model_choice, deactivate_provider
    from pace_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from pace_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `pace model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from pace_cli.auth import _save_model_choice, deactivate_provider
    from pace_cli.config import save_env_value, load_config, save_config
    from pace_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from pace_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from pace_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates PACE turns to `copilot --acp`.")
    print("  PACE currently starts its own ACP subprocess for each request.")
    print("  PACE uses your selected model as a hint for the Copilot ACP session.")
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from pace_constants import display_pace_home as _dhh_fn

            print(
                f"    PACE will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'pace model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate PACE CLI with a provider."""
    from pace_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from pace_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from pace_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from pace_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from pace_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from pace_cli.webhook import webhook_command
...
def cmd_doctor(args):
    ...
    from pace_cli.doctor import run_doctor
...
def cmd_config(args):
    ...
    from pace_cli.config import config_command
...
def cmd_version(args):
    """Show version."""
    print(f"PACE Atlas v{__version__} ({__release_date__})")
    ...
    try:
        from pace_cli.banner import check_for_updates
        from pace_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall PACE Atlas."""
    _require_tty("uninstall")
    from pace_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``pace update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from pace_constants import get_pace_home
...
def _update_via_zip(args):
    """Update PACE Atlas by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/pace-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="pace-update-")
        zip_path = os.path.join(tmp_dir, f"pace-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to pace-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"pace-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "pace-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if PACE behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so PACE can run?")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but PACE couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but PACE couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if PACE behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `pace update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/pace-agent.git",
    "git@github.com:NousResearch/pace-agent.git",
    "https://github.com/NousResearch/pace-agent",
    "git@github.com:NousResearch/pace-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/pace-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_pace_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_pace_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official PACE repository.")
        print("  This means you may miss updates from NousResearch/pace-agent.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/pace-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/pace-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``pace update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".pace"
    homes.append(default_home)
    # Named profiles under ~/.pace/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "pace-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update PACE Atlas to the latest version."""
    import shutil
    from pace_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update PACE Atlas")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/pace-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_pace_home added to pace_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload pace_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_pace_home() added since the last release.
        try:
            import importlib
            import pace_constants as _hc
        ...
        try:
            ...
            other_profiles = [
                p for p in list_profiles() if not p.is_default and p.name != active
            ]
            if other_profiles:
                print()
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'pace config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: pace config migrate")
            else:
                print()
                print("Skipped. Run 'pace config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Discover all pace-gateway* units (default + profiles)
            if is_linux():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. pace-gateway.service or pace-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: pace gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: pace -p <profile> gateway run  for each profile)"
                        )
        ...
        print("  pace model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``pace -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from pace_constants import display_pace_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `pace profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_pace_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → pace -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.pace)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  pace profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     pace -p {name} chat")
                else:
            ...
            if clone or clone_all:
                ...
                profile_dir_display = f"~/.pace/profiles/{name}"
        ...
        from pace_cli.profiles import profile_exists
        ...
        from pace_cli.profiles import rename_profile
        ...
        from pace_cli.profiles import export_profile
        ...
        from pace_cli.profiles import import_profile
...
def cmd_completion(args):
    ...
    from pace_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for pace CLI."""
    parser = argparse.ArgumentParser(
        prog="pace",
        description="PACE Atlas - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    pace                        Start interactive chat
    pace chat -q "Hello"        Single query mode
    pace -c                     Resume the most recent session
    pace -c "my project"        Resume a session by name (latest in lineage)
    pace --resume <session_id>  Resume a specific session by ID
    pace setup                  Run setup wizard
    pace logout                 Clear stored authentication
    pace auth add <provider>    Add a pooled credential
    pace auth list              List pooled credentials
    pace auth remove <p> <t>    Remove pooled credential by index, id, or label
    pace auth reset <provider>  Clear exhaustion status for a provider
    pace model                  Select default model
    pace config                 View configuration
    pace config edit            Edit config in $EDITOR
    pace config set model gpt-4 Set a config value
    pace gateway                Run messaging gateway
    pace -s pace-agent-dev,github-auth
    pace -w                     Start in isolated git worktree
    pace gateway install        Install gateway background service
    pace sessions list          List past sessions
    pace sessions browse        Interactive session picker
    pace sessions rename ID T   Rename/title a session
    pace update                 Update to latest version
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with PACE Atlas",
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: pace-cli)",
    )
    model_parser.add_argument(
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure PACE Atlas with an interactive wizard. "
        "Run a specific section: pace setup model|terminal|gateway|tools|agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for PACE CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: pace-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of PACE Atlas components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with PACE Atlas setup",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage PACE Atlas configuration",
    )
    ...
    def cmd_pairing(args):
        from pace_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from pace_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from pace_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/pace-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    def cmd_plugins(args):
        from pace_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'pace memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'pace memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser(
        "setup", help="Initial Honcho setup (redirects to pace memory setup)"
    )
    honcho_status = honcho_subparsers.add_parser(
        "status", help="Show current Honcho config and connection status"
    )
    honcho_status.add_argument(
        "--all", action="store_true", help="Show config overview across all profiles"
    )
    honcho_subparsers.add_parser(
        "peers", help="Show peer identities across all profiles"
    )
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to PACE Honcho",
    )
    honcho_subparsers.add_parser("enable", help="Enable Honcho for the active profile")
    honcho_subparsers.add_parser(
        "disable", help="Disable Honcho for the active profile"
    )
    honcho_subparsers.add_parser(
        "sync", help="Sync Honcho config to all existing profiles"
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'pace memory setup'...\n")
            from pace_cli.memory_setup import memory_command
    ...
    memory_off_p = memory_sub.add_parser(
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'pace tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # pace tools disable <name...> [--platform cli]
    ...
    # pace tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from pace_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from pace_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run PACE as an MCP server",
        description=(
            "Manage MCP server connections and run PACE as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'pace mcp add' to connect to a new server, or\n"
            "'pace mcp serve' to expose PACE conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run PACE as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from pace_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch pace --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil

            pace_bin = shutil.which("pace")
            if pace_bin:
                os.execvp(pace_bin, ["pace", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "pace_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to PACE",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to PACE",
    ...
    def cmd_claw(args):
        from pace_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update PACE Atlas to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall PACE Atlas",
        description="Remove PACE Atlas from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run PACE Atlas as an ACP (Agent Client Protocol) server",
        description="Start PACE Atlas in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch PACE Atlas as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated PACE instances",
    )
    ...
    # e.g. ``pace -c Pokemon Agent Dev`` → ``pace -c 'Pokemon Agent Dev'``
