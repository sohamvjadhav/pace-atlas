...
from pace_cli.auth import get_nous_auth_status
from pace_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: pace setup tools, pace setup terminal, or pace status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `pace setup` and `pace model`."""
    if not managed_nous_tools_enabled():
        return set()
