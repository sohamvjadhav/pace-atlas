# PACE Atlas Security Policy

This document describes PACE Atlas's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/sohamvjadhav/pace-atlas/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **PACE Atlas does not operate a bug
bounty program.**
...
- Environment details (`pace version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted PACE Atlas access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in PACE Atlas's documentation
...
PACE Atlas supports two OS-level isolation postures. They address
...
PACE Atlas supports this in two ways:

- **PACE Atlas's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, PACE Atlas's in-process heuristics
...
A malicious or buggy plugin is not a vulnerability in PACE Atlas
itself. Bugs in PACE Atlas's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   PACE Atlas does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, PACE Atlas's own documentation, or reasonable
  operator expectations would predict — including cases where
  PACE Atlas has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not PACE Atlas's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in PACE Atlas. Bugs in Hermes
...
- PACE Atlas includes supply-chain guards for MCP server
