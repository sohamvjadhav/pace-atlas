...
That ordering matters because Hermes treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `pace model`.
...
- custom endpoints saved via `pace model` / `config.yaml` also work
...
- `tests/pace_cli/test_fallback_cmd.py` — the `/fallback` CLI command
