...
pace-agent itself). For the async surface (`acomplete()` /
  ...
  entries:
    my-plugin:
      llm:
        ...
        # `pace model` and config.yaml model.provider).
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.pace/auth.json` / env, including the credential pool when
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/pace-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/pace-agent/blob/main/tests/agent/test_plugin_llm.py)
