...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `pace plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `pace memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `pace memory setup`:
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.pace`:
...
data_dir = Path("~/.pace/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `pace my-provider status`, `pace my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `pace <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `pace --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: pace my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the pace my-provider argparse tree.
