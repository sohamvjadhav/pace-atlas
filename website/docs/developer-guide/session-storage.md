...
Hermes Agent uses a SQLite database (`~/.pace/state.db`) to persist session
...
Multiple pace processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.pace/state.db
...
Default path: `~/.pace/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.pace/` by default, or the value of `HERMES_HOME` environment variable.
