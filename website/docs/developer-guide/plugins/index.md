...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.pace/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `pace skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.pace/plugins/calculator
cd ~/.pace/plugins/calculator
```
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `pace my-plugin <subcommand>`)
...
```bash
HERMES_PLUGINS_DEBUG=1 pace plugins list
```
...
The same logs are always written to `~/.pace/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
pace logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `pace plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.pace/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.pace/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.pace/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.pace/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `pace plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
Plugins can add their own `pace <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for pace my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: pace my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for pace my-plugin."""
...
After registration, users can run `pace my-plugin status`, `pace my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `pace name` in a terminal |
...
Hermes has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.pace/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `pace gateway status` without SDK instantiation.
...
Model Context Protocol (MCP) servers register their own tools into Hermes without any Python plugin. Declare them in `~/.pace/config.yaml`:
...
Drop a manifest + handler into `~/.pace/hooks/<name>/`:
...
pace skills search my-workflow --source myorg/skills-repo
pace skills install myorg/skills-repo/my-workflow
```
...
# Plugin auto-discovered on next pace startup
```
