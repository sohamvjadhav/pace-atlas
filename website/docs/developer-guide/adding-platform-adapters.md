...
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.pace/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `pace config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
| `pace status` | Shows plugin platforms with `(plugin)` tag |
| `pace gateway setup` | Plugin platforms appear in setup menu |
| `pace tools` / `pace skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.pace/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `pace gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `pace cron run` separate from `pace gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `pace config`

`pace_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #pace)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
1. **`pace_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`pace_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`pace_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`pace_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`pace_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`pace_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
