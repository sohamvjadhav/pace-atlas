...
If your tool requires an API key, add it to `pace_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `pace_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `pace chat -q "Use the weather tool for London"`
