...
| `pace_cli/cron.py` | CLI `pace cron` subcommands |
...
Jobs are stored in `~/.pace/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `pace cron` commands are run or during active CLI sessions.
...
```python
# ~/.pace/scripts/check_competitors.py
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `pace cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `pace cron` CLI provides direct job management:

```bash
pace cron list                    # Show all jobs
pace cron create                  # Interactive job creation (alias: add)
pace cron edit <job_id>           # Edit job configuration
pace cron pause <job_id>          # Pause a running job
pace cron resume <job_id>         # Resume a paused job
pace cron run <job_id>            # Trigger immediate execution
pace cron remove <job_id>         # Delete a job
```
