...
2. **context** — caller-supplied `system_message` plus project context files (`.pace.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.pace/SOUL.md)
...
`SOUL.md` lives at `~/.pace/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .pace.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.pace.md`, `HERMES.md` | CWD up to git root | Hermes-native project config |
...
1. `.pace.md` / `HERMES.md` (walks to git root)
...
- `~/.pace/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.pace/MEMORY.md` and `~/.pace/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.pace.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
