...
```text
pace acp / hermes-acp / python -m acp_adapter
...
- ACP sessions are persisted to the shared `~/.pace/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `pace_cli/main.py` — `pace acp` CLI subcommand
