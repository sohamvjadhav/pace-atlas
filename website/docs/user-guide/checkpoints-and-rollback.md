...
```bash
pace chat --checkpoints
```

Or enable globally in `~/.pace/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.pace/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `pace checkpoints` | Show total size, project count, per-project breakdown |
| `pace checkpoints status` | Same as bare `checkpoints` |
| `pace checkpoints list` | Alias for `status` |
| `pace checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `pace checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `pace checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |
...
- Once per conversation turn (per directory), it:
  ...
  - Initialises or reuses the **single shared shadow store** at `~/.pace/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/pace/<project-hash>`).
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `pace checkpoints prune` manually.
...
```text
Checkpoint base: /home/you/.pace/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/pace-agent                                        20       2h ago  live
...
Clear with: pace checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
pace checkpoints prune --retention-days 3 --max-size-mb 200
```
...
~/.pace/checkpoints/
  ...
  │   ├── refs/pace/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `pace checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.pace/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.pace/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
pace checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `pace chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `pace checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
