...
sidebar_position: 14
title: "API Server"
description: "Expose pace-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes pace-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to pace-agent and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
Hermes itself needs a configured provider and tool backends for the API server to be useful. A [Nous Portal](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `pace setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "pace-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus Hermes' custom `pace.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: Hermes emits `event: pace.tool.progress` for tool-start visibility without polluting persisted assistant text.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `pace-agent` for the default profile). Required by most frontends for model discovery.
...
```json
{
  "object": "pace.api_server.capabilities",
  "platform": "pace-agent",
  "model": "pace-agent",
...
Create a new scheduled job. Body accepts the same shape as `pace cron` — prompt, schedule, skills, provider override, delivery target.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), pace-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
:::warning Security
The API server gives full access to pace-agent's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `pace-agent` for default profile. |
...
pace profile create alice
pace profile create bob
...
cat >> ~/.pace/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8643
API_SERVER_KEY=alice-secret
EOF

cat >> ~/.pace/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
