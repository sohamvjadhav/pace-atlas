...
persona in `~/.pace/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.pace/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.pace/config.yaml` under the `mcp_servers`
