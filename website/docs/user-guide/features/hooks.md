...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.pace/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.pace/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".pace" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.pace/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.pace/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.pace/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.pace/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".pace" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.pace/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.pace/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.pace/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.pace/BOOT.md` with natural-language startup instructions.
...
Create `~/.pace/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.pace/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.pace/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.pace/hooks/boot-md/handler.py`**

```python
"""Run ~/.pace/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".pace" / "BOOT.md"
...
```bash
pace logs --follow --level INFO | grep boot-md
```
...
Delete `~/.pace/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.pace/hooks/`
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`pace_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.pace/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.pace/agent-hooks/` (by convention) | `~/.pace/plugins/<name>/` | `~/.pace/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.pace/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.pace/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.pace/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.pace/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.pace/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.pace/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.pace/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.pace/agent-hooks/log-orchestration.sh
log=~/.pace/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.pace/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `pace --accept-hooks chat`)
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `pace hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.
...
|---------|--------------|
| `pace hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `pace hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `pace hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `pace hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.pace/agent-hooks/` so the path is easy to audit.
- Re-run `pace hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
