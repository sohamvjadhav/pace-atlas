...
For Hermes profile lanes, the dispatcher's `_default_spawn` runs `pace -p <assignee> chat -q <prompt>` (or the equivalent module form when the `pace` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:
...
- **Block instead of complete**, with `reason` prefixed `review-required: ` so the dashboard / `pace kanban show` surfaces the row as awaiting review.
...
The dashboard renders run history with summaries, metadata blocks, and exit-status badges. CLI users can run `pace kanban tail <task_id>` to follow live, or `pace kanban runs <task_id>` for the historical attempt list.
...
The historical issue for this is [#19931](https://github.com/NousResearch/pace-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/NousResearch/pace-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
