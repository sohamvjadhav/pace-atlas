...
the refresh token in `~/.pace/auth.json` — the same place all Hermes
...
```bash
pace proxy start --host 0.0.0.0 --port 8645
```
...
`pace_cli/proxy/adapters/<provider>.py` and registering it in
