...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Hermes](/guides/use-voice-mode-with-pace).
...
:::tip
The `~/.pace/` directory and default `config.yaml` are created automatically the first time you run `pace`. You only need to create `~/.pace/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `pace setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`pace chat`) and the **TUI** (`pace --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.pace/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
pace                # Start the interactive CLI
```
...
1. Start the CLI with `pace` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.pace/config.yaml` (default: `ctrl+b`).
:::
...
```bash
pace gateway        # Start the gateway (connects to configured platforms)
pace gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.pace/.env`:
```bash
...
```bash
pace gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.pace/.env`
...
- Review gateway logs: `tail -f ~/.pace/logs/gateway.log`
