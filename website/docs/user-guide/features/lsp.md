...
run `pace lsp install typescript` or auto-install fires on first
use.

## CLI

```
pace lsp status          # service state + per-server install status
pace lsp list            # registry, optionally --installed-only
pace lsp install <id>    # eagerly install one server
pace lsp install-all     # try every server with a known recipe
pace lsp restart         # tear down running clients
pace lsp which <id>      # print resolved binary path
```

`pace lsp status` is the best starting point — it shows which
...
**`pace lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`pace lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `pace lsp status`**
...
When `pace lsp status` shows a `Backend warnings` section, install
...
Check `~/.pace/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `pace lsp restart` to clear the set;
