...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `pace setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `pace model` or `pace tools`.
:::
...
mode, so you need it installed (`pace setup tools → Browser Automation`
...
Or configure via `pace tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.pace/config.yaml`:
...
brave-browser \
  ...
  --user-data-dir=$HOME/.pace/chrome-debug \
  --no-first-run \
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `pace config set toolsets '["hermes-cli", "browser"]'`.
:::
...
```
Type "pace agent" into the search field @e3
```
...
Screenshots are stored in `~/.pace/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.pace/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
