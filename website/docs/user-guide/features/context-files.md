...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .pace.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---
...
|------|---------|-----------| 
| **.pace.md** / **HERMES.md** | Project instructions (highest priority) | Walks to git root |
...
Only **one** project context type is loaded per session (first match wins): `.pace.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
Hermes is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.pace.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `.pace.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
