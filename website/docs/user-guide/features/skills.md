...
All skills live in **`~/.pace/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
By default every profile is seeded with the bundled skill catalog, and each `pace update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.pace` profile):

```bash
curl -fsSL https://pace-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
pace profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
pace skills opt-out            # stop future seeding — nothing on disk is touched
pace skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
pace skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `pace update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `pace skills opt-in`) to re-enable.

:::note Safe by default
`pace skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Hermes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
```bash
pace chat --toolsets skills -q "What skills do you have?"
pace chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.pace/cache/chart-q4-2025.png
...
When a missing value is encountered, Hermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `pace setup` or `~/.pace/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `pace config migrate` prompts for unconfigured settings, and `pace config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.pace/skills/                  # Single source of truth
...
Add `external_dirs` under the `skills` section in `~/.pace/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.pace/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
...
```text
~/.pace/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.pace/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `pace bundles list`.
...
pace bundles create research

# Overwrite an existing bundle
pace bundles create backend-dev --skill ... --force

# Delete a bundle
pace bundles delete backend-dev

# Re-scan ~/.pace/skill-bundles/ and report changes
pace bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.pace/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.pace/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `pace skills search <query> --source skills-sh`. Hermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
```bash
pace skills browse --source official
pace skills install official/security/1password
```
...
```bash
pace skills search react --source skills-sh
pace skills inspect skills-sh/vercel-labs/json-render/json-render-react
pace skills install skills-sh/vercel-labs/json-render/json-render-react --force
```
...
```bash
pace skills search https://mintlify.com/docs --source well-known
pace skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
pace skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
pace skills install openai/skills/k8s
pace skills tap add myorg/skills-repo
```
...
[Skills Hub](https://pace-agent.nousresearch.com/docs) page — instead of a
...
```bash
pace skills search airbnb --source browse-sh
pace skills inspect browse-sh/airbnb.com/search-listings-ddgioa
pace skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
```bash
pace skills install https://sharethis.chat/SKILL.md
pace skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
pace skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `pace skills update` re-fetches from the same URL automatically when you want to refresh.
...
`pace skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
pace skills install skills-sh/anthropics/skills/pdf --force
```
...
```bash
pace skills check          # Report which installed hub skills changed upstream
pace skills update         # Reinstall only the skills with updates available
pace skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Hermes users add with `pace skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.pace.tags`, `version`, `author`, `platforms`, `metadata.pace.config`).
...
```bash
pace skills tap add my-org/hermes-skills
pace skills search deploy
pace skills install my-org/hermes-skills/deploy-runbook
```
...
The `pace skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `pace skills tap list` shows the effective path per tap.
...
```bash
pace skills install owner/repo/skills/my-workflow
```
...
```bash
pace skills tap list                                # show all configured taps
pace skills tap add myorg/skills-repo               # add (default path: skills/)
pace skills tap remove myorg/skills-repo            # remove
```
...
## Bundled skill updates (`pace skills reset`)

Hermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `pace update`, a sync pass copies those into `~/.pace/skills/` and records a manifest at `~/.pace/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `pace -p coder skills reset <name>` only affects that profile.
:::
