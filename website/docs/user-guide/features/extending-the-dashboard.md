...
The Hermes web dashboard (`pace dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.pace/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.pace/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.pace/dashboard-themes
```

```yaml
# ~/.pace/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.pace/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.pace/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.pace/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.pace/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
```json
// ~/.pace/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.pace/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.pace/plugins/my-plugin/dashboard/plugin_api.py
...
Backend routes run inside the dashboard process, so they can import from the pace-agent codebase directly:

```python
from fastapi import APIRouter
from hermes_state import SessionDB
from pace_cli.config import load_config
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.pace/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.pace/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.pace/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.pace/plugins/
```
...
Check that the file is in `~/.pace/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.pace/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.pace/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `pace dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.pace/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.pace/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
