...
Unlike Hermes' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `pace auth spotify` walks you through it.
...
### One-shot: `pace tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `pace setup` / `pace setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `pace tools`.
...
4. Saves it to `~/.pace/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.pace/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
| Field | Value |
|-------|-------|
| App name | anything (e.g. `pace-agent`) |
...
```bash
pace auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Hermes restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `pace auth logout spotify`.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `pace tools` for the cron session to see the Spotify tools.
...
```bash
pace auth logout spotify
```

Removes tokens from `~/.pace/auth.json`. To also clear the app config, delete `HERMES_SPOTIFY_CLIENT_ID` (and `HERMES_SPOTIFY_REDIRECT_URI` if you set it) from `~/.pace/.env`, or run the wizard again.
...
**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Hermes is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `HERMES_SPOTIFY_REDIRECT_URI` in `~/.pace/.env` to whatever you registered.
...
**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `pace auth spotify` again.
...
```bash
pace auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
pace auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.pace/.env`:
...
| File | Contents |
|------|----------|
| `~/.pace/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.pace/.env` | `HERMES_SPOTIFY_CLIENT_ID`, optional `HERMES_SPOTIFY_REDIRECT_URI` |
