...
Drop a directory into `~/.pace/plugins/` with a `plugin.yaml` and Python code:

```
~/.pace/plugins/my-plugin/
...
**`~/.pace/plugins/hello-world/plugin.yaml`**
...
**`~/.pace/plugins/hello-world/__init__.py`**
...
Drop both files into `~/.pace/plugins/hello-world/`, restart Hermes, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.pace/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Hermes.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `pace <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `pace plugins install` |
...
| User | `~/.pace/plugins/` | Personal plugins |
| Project | `.pace/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hermes_agent.plugins` entry_points | Distributed packages |
| Nix | `services.pace-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
User plugins at `~/.pace/plugins/model-providers/<name>/` and `~/.pace/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `pace plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.pace/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
pace plugins                    # interactive toggle (space to check/uncheck)
pace plugins enable <name>      # add to allow-list
pace plugins disable <name>     # remove from allow-list + add to disabled
```

After `pace plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.
...
| **User-installed platforms** (under `~/.pace/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.pace/plugins/`.

### Migration for existing users

When you upgrade to a version of Hermes that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.pace/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.pace/plugins/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `pace skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.pace/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `pace plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
pace plugins                               # unified interactive UI
pace plugins list                          # table: enabled / disabled / not enabled
pace plugins install user/repo             # install from Git, then prompt Enable? [y/N]
pace plugins install user/repo --enable    # install AND enable (no prompt)
pace plugins install user/repo --no-enable # install but leave disabled (no prompt)
pace plugins update my-plugin              # pull latest
pace plugins remove my-plugin              # uninstall
pace plugins enable my-plugin              # add to allow-list
pace plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `pace plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `pace plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
