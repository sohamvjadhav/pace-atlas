...
/skin mytheme        # switch to a custom skin from ~/.pace/skins/mytheme.yaml
```

Or set the default skin in `~/.pace/config.yaml`:
...
Create YAML files under `~/.pace/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.pace/skins/mytheme.yaml
...
- Saves directly to `~/.pace/skins/`
- Activates a skin by updating `~/.pace/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.pace/skins/`.
...
- Built-in skins load from `pace_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.pace/skins/` take precedence over built-in skins with the same name.
