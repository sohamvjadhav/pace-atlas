...
2. Add an MCP server to `~/.pace/config.yaml`:
...
```bash
pace mcp                # interactive picker (default)
pace mcp catalog        # plain-text list, scriptable
pace mcp install n8n    # install a catalog entry by name
```
...
`optional-mcps/` in the pace-agent repo — presence in that directory means
...
- **API key** — Hermes prompts at install time and writes the value to
  `~/.pace/.env`. Non-secret values (base URLs) go to the same file.
...
- **OAuth** (third-party provider like Google/GitHub) — Hermes points you at
  `pace auth <provider>` if you haven't authenticated already.
...
written (if not). Re-run `pace mcp configure <name>` once the server is
reachable to refine.
...
the pace-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/pace-agent/tree/main/optional-mcps)
...
Hermes`) for that entry instead of silently hiding it. Run `pace update`
...
from environment variables (which include everything in `~/.pace/.env`).
...
MCPs are never auto-updated. Re-run `pace mcp install <name>` to refresh
after a Hermes update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NousResearch/pace-agent/tree/main/optional-mcps).
...
On first connect, Hermes prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.pace/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `pace mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `pace mcp login` can list the tools and look like it worked, but every real tool call later times out. `pace mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `pace mcp login googledrive` — with the pre-registered client, Hermes skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.pace/config.yaml` from inside a running Hermes session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `pace mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.
...
Hermes reads MCP config from `~/.pace/config.yaml` under `mcp_servers`.
...
For well-known MCP servers, `pace mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
pace mcp add codex --preset codex
```
...
You can pick any local name (`pace mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.
...
```json
{
  "mcpServers": {
    "pace": {
      "command": "/home/user/.pace/pace-agent/venv/bin/pace",
      "args": ["mcp", "serve"]
    }
  }
}
...
```bash
pace mcp serve              # Normal mode
pace mcp serve --verbose    # Debug logging on stderr
```
...
- The embedded `pace mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Hermes, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
