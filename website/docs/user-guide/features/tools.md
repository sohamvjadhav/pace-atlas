...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `pace tools` → 🐦 X (Twitter) Search. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `pace model` to enable it, or configure individual tools with `pace tools`.
:::
...
pace chat --toolsets "web,terminal"
...
# Configure
pace config set terminal.backend singularity
pace config set terminal.singularity_image ~/python.sif
```
...
modal setup
pace config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.pace/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.pace/.env`.
:::
