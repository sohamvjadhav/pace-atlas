...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\pace`, WSL data lives under `~/.pace`.
:::
...
```powershell
iex (irm https://raw.githubusercontent.com/NousResearch/pace-agent/main/scripts/install.ps1)
```

No admin rights required. The installer goes to `%LOCALAPPDATA%\pace\` and adds `pace` to your **User PATH** — open a new terminal after it finishes.

**Installer options** (requires the scriptblock form to pass parameters):

```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/NousResearch/pace-agent/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
| `-SkipSetup` | off | Skip the post-install `pace setup` wizard |
| `-HermesHome` | `%LOCALAPPDATA%\pace` | Override data directory |
| `-InstallDir` | `%LOCALAPPDATA%\pace\pace-agent` | Override code location |
...
On first launch (and on demand when a missing tool is detected), Hermes runs a small Python bootstrapper — `pace_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:
...
|---|---|---|
| CLI (`pace chat`, `pace setup`, `pace gateway`, …) | ✓ | ✓ |
| Interactive TUI (`pace --tui`) | ✓ | ✓ |
...
2. `%LOCALAPPDATA%\pace\git\usr\bin\bash.exe` (installer-managed PortableGit).
3. `%LOCALAPPDATA%\pace\git\bin\bash.exe` (older Git-for-Windows layout).
...
The fix is in `pace_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `pace_cli/main.py::main`, `gateway/run.py::main`). It:
...
`pace gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.

### Install

```powershell
pace gateway install
```
...
```powershell
pace gateway status      # Merged view: schtasks + Startup folder + running PID
pace gateway start       # Starts the scheduled task now
pace gateway stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
pace gateway restart
pace gateway uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`pace gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)
...
- Playwright Chromium is auto-installed on first run (`npx playwright install chromium`). If installation fails, `pace doctor` surfaces it with a fix-it hint.
...
The `pace uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
...
**`pace: command not found` right after install.**
Open a new PowerShell window. The installer added `%LOCALAPPDATA%\pace\bin` to User PATH, but existing shells need to be restarted to pick it up. In the meantime you can run `& "$env:LOCALAPPDATA\pace\bin\pace.cmd"`.
...
Chromium is auto-installed on first run. If the install failed (rate-limited GitHub, Playwright CDN hiccup), run `pace doctor` — it will surface the missing Chromium and print the exact `npx playwright install chromium` command to fix it.
...
- **[CLI Reference](../reference/cli-commands.md)** — every `pace` subcommand.
