...
One command — `pace setup --portal` — and you're ready to `pace chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Hermes also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `pace --tui` — see the [TUI](tui.md) guide.
:::
...
pace chat --model "anthropic/claude-sonnet-4"

# With a specific provider
pace chat --provider nous        # Use Nous Portal
pace chat --provider openrouter  # Force OpenRouter

# With specific toolsets
pace chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
pace -s pace-agent-dev,github-auth
pace chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
pace --continue             # Resume the most recent CLI session (-c)
pace --resume <session_id>  # Resume a specific session by ID (-r)
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `pace --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`pace -c` or `pace --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
```bash
pace -s pace-agent-dev,github-auth
pace chat -s github-pr-workflow -s github-auth
```
...
Every installed skill in `~/.pace/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
Resume this session with:
  pace --resume 20260225_143052_a1b2c3
...
```bash
pace --continue                          # Resume the most recent CLI session
pace -c                                  # Short form
pace -c "my project"                     # Resume a named session (latest in lineage)
pace --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
pace --resume "refactoring auth"         # Resume by title
pace -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `pace sessions rename <id> <title>` from the command line. Use `pace sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Hermes's SQLite state database under `~/.pace/state.db`. The database keeps:
