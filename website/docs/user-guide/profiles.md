...
```bash
pace profile create coder       # creates profile + "coder" command alias
...
:::tip
Quickest setup: run `pace setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
pace profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `pace profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
The alias works with every pace subcommand — it's just `pace -p <name>` under the hood.
...
```bash
pace -p coder chat
pace --profile=coder doctor
pace chat -p coder -q "hello"    # works in any position
```

### Sticky default (`pace profile use`)

```bash
pace profile use coder
pace chat                   # now targets coder
pace tools                  # configures coder's tools
pace profile use default    # switch back
```

Sets a default so plain `pace` commands target that profile. Like `kubectl config use-context`.
...
- **`pace profile`**: Shows current profile name, path, model, gateway status
...
```bash
# Edit coder's tokens
nano ~/.pace/profiles/coder/.env

# Edit assistant's tokens
nano ~/.pace/profiles/assistant/.env
```
...
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `pace profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `pace -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.pace/profiles/coder/SOUL.md
```
...
`pace update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
pace profile list           # show all profiles with status
pace profile show coder     # detailed info for one profile
pace profile rename coder dev-bot   # rename (updates alias + service)
pace profile export coder   # export to coder.tar.gz
pace profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
pace profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `pace profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.pace`). To remove everything, use `pace uninstall`.
:::
...
```bash
# Bash
eval "$(pace completion bash)"

# Zsh
eval "$(pace completion zsh)"
```
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.pace/profiles/coder` before launching pace. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Hermes state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
