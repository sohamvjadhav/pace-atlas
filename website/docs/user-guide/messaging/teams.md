...
> Run `pace gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Alternatively, configure via `~/.pace/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.pace/.env` |
...
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs pace` for tracebacks |
...
- Store credentials in `~/.pace/.env` with permissions `600` (`chmod 600 ~/.pace/.env`)
