...
2. Choose a username like `pace` — the full user ID will be `@pace:your-server.org`.
...
```bash
MATRIX_USER_ID=@pace:your-server.org
...
Add the following to your `~/.pace/.env` file:
...
# MATRIX_USER_ID=@pace:matrix.example.org
...
:::tip
You can run `pace gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::
...
- Stores encryption keys in `~/.pace/platforms/matrix/store/` (legacy installs: `~/.pace/matrix/store/`)
...
If you delete `~/.pace/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@pace:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@pace:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@pace:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@pace:your-server';
   "
...
2. Delete the local crypto store and restart Hermes:
   ```bash
   rm -f ~/.pace/platforms/matrix/store/crypto.db*
   # restart pace
   ```
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/pace-agent/issues/12614).
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@pace:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Hermes Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.pace/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.pace/platforms/matrix/store/crypto.db
   rm -f ~/.pace/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.pace/.env`:
...
services:
  hermes-matrix:
    ...
    volumes:
      - ./matrix-store:/root/.pace/platforms/matrix/store
```
...
**Fix**: Check that `pace gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.pace/.env` and restart the gateway. Use the full `@user:server` format.
