...
Configure which events the agent sees in `~/.pace/config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/.pace/.env` (auto-merged at startup) or
