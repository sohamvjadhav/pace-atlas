...
5. Save the account credentials automatically to `~/.pace/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.pace/.env`:
...
- **QR code login** — scan-to-connect setup via `pace gateway setup`
...
1. Pair Hermes once with `pace gateway setup` and note the connected iLink bot
   account.
...
- Tokens are saved per account+peer to `~/.pace/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `pace gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `pace gateway setup` |
...
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `pace gateway setup` to scan a new QR code |
