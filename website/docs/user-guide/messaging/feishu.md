...
5. Run `pace gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `pace update --gateway` needs confirmation, the adapter records the selected answer in Hermes's `.update_response` file and replaces the card inline with a resolved state.
...
Rules live in `~/.pace/feishu_comment_rules.json` (pairing grants in `~/.pace/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.pace/feishu_seen_message_ids.json`.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `pace gateway setup` |
