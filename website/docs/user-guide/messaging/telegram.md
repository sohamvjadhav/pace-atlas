...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.pace/config.yaml`:
...
```
[The user sent a voice message: /home/<user>/.pace/cache/audio/<hash>.ogg]
```
...
Add the URLs under `platforms.telegram.extra` in `~/.pace/config.yaml`:
...
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.pace/logs/gateway.log | tail
```
...
```bash
tail -f ~/.pace/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.pace/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.
...
# named profiles
pace -p research gateway start
pace -p research gateway status
pace -p research gateway stop
```

For a small fixed fleet, use a shell loop or script that calls `pace gateway <action>` for the default profile and `pace -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/.pace/config.yaml`:
...
When this flag is on, Hermes still generates an internal session title (used by `pace sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.pace/config.yaml`:
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `pace gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.pace/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.pace/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
