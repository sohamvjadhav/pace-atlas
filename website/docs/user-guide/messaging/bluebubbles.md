...
Or set environment variables directly in `~/.pace/.env`:
...
```bash
pace pairing approve bluebubbles <CODE>
```
Use `pace pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.pace/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.pace/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.pace/config.yaml` (default: `true`). There is no corresponding environment variable.
...
- Check `pace logs gateway` for webhook errors (or `pace logs -f` to follow in real-time)
