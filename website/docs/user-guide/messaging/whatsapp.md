...
> Run `pace gateway setup` and pick **WhatsApp** for a guided walk-through.
...
3. Run `pace whatsapp` and scan the QR code from that WhatsApp account
...
Optional behavior settings in `~/.pace/config.yaml`:
...
```bash
pace gateway              # Foreground
pace gateway install      # Install as a user service
sudo pace gateway install --system   # Linux only: boot-time system service
```
...
The Baileys bridge saves its session under `~/.pace/platforms/whatsapp/session`. This means:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `pace whatsapp`. |
| **Session not persisting** | Check that `~/.pace/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `pace whatsapp` if needed. |
...
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `pace gateway install` to re-snapshot your current PATH into the plist, then `pace gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.pace/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.pace/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.pace/platforms/whatsapp/session`
