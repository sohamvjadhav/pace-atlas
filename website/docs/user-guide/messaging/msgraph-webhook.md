...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.pace/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.pace/config.yaml`:
...
Or via env vars in `~/.pace/.env` (auto-merged on startup):
...
Start the gateway: `pace gateway run`. The listener exposes:
...
| Every notification 403s | `clientState` mismatch (forged, or subscription registered with a different value). Re-create the subscription with `pace teams-pipeline subscribe --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE" ...` (ships with the pipeline runtime PR). |
