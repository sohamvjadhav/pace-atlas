...
Discord behavior is controlled through two files: **`~/.pace/.env`** for credentials and env-level toggles, and **`~/.pace/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
The `discord` section in `~/.pace/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
No extra configuration is needed — any skill installed via `pace skills install` is automatically registered as a Discord slash command on the next gateway restart.
...
- [Use Voice Mode with Hermes](/guides/use-voice-mode-with-pace)
...
**Fix**: Check that `pace gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.pace/.env` and restart the gateway.
...
**Fix**: Set this in `~/.pace/config.yaml` and restart the gateway:
...
```bash
# ~/.pace/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.pace/.env — env vars win over config.yaml
