...
flowchart LR
    ...
    B["pace-agent<br/>gateway API server<br/>port 8642"]
...
```bash
pace config set API_SERVER_ENABLED true
pace config set API_SERVER_KEY your-secret-key
```

`pace config set` auto-routes the flag to `config.yaml` and the secret to `~/.pace/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
pace gateway stop && pace gateway
```
...
# {"object":"list","data":[{"id":"pace-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **pace-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **pace-agent** for the default profile).
...
2. Edit your pace-agent connection
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `pace-agent`
...
```bash
pace -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
