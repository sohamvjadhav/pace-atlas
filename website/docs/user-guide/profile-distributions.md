...
```bash
pace profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `pace profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
- **You just want to back up a profile on your own machine.** Use [`pace profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — that's what those are for.
...
```bash
pace profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.pace/profiles/research-bot/SOUL.md
...
Create `~/.pace/profiles/research-bot/distribution.yaml`:
...
Recipients who run `pace profile update research-bot` will pull the latest.
...
```bash
pace profile install github.com/you/research-bot --alias
```
...
```bash
# GitHub shorthand
pace profile install github.com/you/research-bot

# Full HTTPS
pace profile install https://github.com/you/research-bot.git

# SSH
pace profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
pace profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
pace profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
pace profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
pace profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
pace profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.pace/profiles/research-bot/.env.EXAMPLE ~/.pace/profiles/research-bot/.env
...
```bash
pace profile info research-bot
```
...
`pace profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
pace profile update research-bot
```
...
```bash
pace profile delete research-bot
```
...
Path:    ~/.pace/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `pace profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
pace profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `pace profile update pr-reviewer` and everyone's on the new version within minutes.
...
# Anyone
pace profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
pace profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
pace profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `pace profile info <name>` and hold off on updates until you're ready.
:::
...
pace profile info research-bot | grep Version
...
```yaml
# ~/.pace/profiles/research-bot/local/my-overrides.yaml
...
pace profile delete research-bot --yes
pace profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
pace profile update research-bot --force-config --yes
```
...
pace profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.pace/profiles/forked-research-bot/
...
pace profile install ~/.pace/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
pace profile delete research-bot-test --yes
pace profile install ~/.pace/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `pace -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `pace profile` — not a parallel command tree.
...
- Reserved profile names (`pace`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`pace profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Hermes](../guides/use-soul-with-pace.md) — authoring personalities
