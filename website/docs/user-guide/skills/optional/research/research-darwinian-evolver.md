...
| | |
|---|---|
| Source | Optional — install with `pace skills install official/research/darwinian-evolver` |
...
```bash
mkdir -p ~/.pace/cache/darwinian-evolver && cd ~/.pace/cache/darwinian-evolver
...
```bash
cd ~/.pace/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.pace/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.pace/skills/research/darwinian-evolver
DE_DIR=~/.pace/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.pace/skills/...` are user-side and fine.
