...
| | |
|---|---|
| Source | Optional — install with `pace skills install official/migration/openclaw-migration` |
...
```bash
pace claw migrate              # Full interactive migration
pace claw migrate --dry-run    # Preview what would be migrated
pace claw migrate --preset user-data   # Migrate without secrets
pace claw migrate --overwrite  # Overwrite existing conflicts
pace claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `pace setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- copy OpenClaw skills into `~/.pace/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.pace/tts/`
...
- `~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.pace/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.pace/skills/migration/openclaw-migration/`.
...
```bash
python3 ~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.pace/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.pace/skills/openclaw-imports/`
