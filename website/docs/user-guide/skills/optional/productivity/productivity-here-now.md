...
| | |
|---|---|
| Source | Optional — install with `pace skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client pace
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client pace
```
...
| `--client {name}`      | Agent name for attribution (e.g. `pace`)    |
