...
- A long-running process (pace gateway, tui_gateway) misbehaves and you can't restart it
...
The pace test runner and pytest both support this:
...
```bash
source /home/bb/pace-agent/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/pace-agent" }
  ]
}
```
...
Easiest: add `breakpoint()` near the suspect line, then run `pace` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `pace --tui`)
...
debugpy.wait_for_client()
```
Start `pace --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
