...
1. **User-local:** `~/.pace/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `/home/bb/pace-agent/skills/<category>/<name>/SKILL.md` — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with pace-agent
- You're editing an existing skill under `/home/bb/pace-agent/skills/` (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)
...
`metadata.pace.related_skills` unions both trees (`skills/` in-repo and `~/.pace/skills/`) at load time. You CAN reference a user-local skill from an in-repo skill, but it won't resolve for other users who clone the repo fresh. Prefer referencing only in-repo skills from in-repo skills. If a frequently-referenced skill lives only in `~/.pace/skills/`, consider promoting it to the repo.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.pace/skills/`, not the repo tree. Use `write_file` for in-repo creation.
...
- [ ] File is at `skills/<category>/<name>/SKILL.md` (not in `~/.pace/skills/`)
- [ ] Frontmatter starts at byte 0 with `---`, closes with `\n---\n`
- [ ] `name`, `description`, `version`, `author`, `license`, `metadata.pace.{tags, related_skills}` all present
