...
| Tags | `debugging`, `pace-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
<!-- ascii-guard-ignore -->
```
Python backend (pace_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path pace_cli/
   search_files --pattern "commandname" --path pace_cli/commands.py --context 3
   ```
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `pace_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/pace-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.pace/config.yaml`)
