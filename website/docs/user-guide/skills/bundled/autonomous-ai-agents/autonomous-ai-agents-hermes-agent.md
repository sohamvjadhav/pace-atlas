...
**Docs:** https://pace-agent.nousresearch.com/docs/
...
# Single query
pace chat -q "What is the capital of France?"
...
| | `delegate_task` | Spawning `pace` process |
|-|-----------------|--------------------------|
...
```
terminal(command="pace chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="pace chat -q 'Set up CI/CD for ~/myapp'", background=true)
```
...
```
# Start
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'pace'", timeout=10)
...
```
# Agent A: backend
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'pace -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B: frontend
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'pace -w'", timeout=10)
...
terminal(command="tmux new-session -d -s resumed 'pace --continue'", timeout=10)

# Resume specific session
terminal(command="tmux new-session -d -s resumed 'pace --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **Use `pace chat -q` for fire-and-forget** — no PTY needed
