...
The pace gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
- Your Hermes install (`~/.pace/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Hermes workflow (`pace chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-pace#wsl2-bridge-hermes-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Hermes's current working directory is `~`. In that case, start Hermes from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
- Using the **OpenAI-compatible API server** (exposed by `pace gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
If you enabled systemd per the setup section above, `pace gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
**Massive slowness on `git status` / `pace chat` in a repo.**
...
**`pace` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-pace#wsl2-bridge-hermes-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Hermes in WSL.
