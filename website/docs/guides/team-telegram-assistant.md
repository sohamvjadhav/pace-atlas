...
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.pace/.env`
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `pace gateway install` to update the plist.
:::
...
```bash
pace gateway stop && pace gateway start
```
...
pace pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.pace/.env`:
...
Control how much detail the bot shows when using tools. In `~/.pace/config.yaml`:
...
Customize how the bot communicates by editing `~/.pace/SOUL.md`:

For a full guide, see [Use SOUL.md with Hermes](/guides/use-soul-with-pace).
...
```bash
# From the CLI
pace cron list          # View all scheduled jobs
pace cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.pace/logs/gateway.log` (macOS) |
| Cron job output | `~/.pace/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.pace/cron/jobs.json` |
| Pairing data | `~/.pace/pairing/` |
| Session history | `~/.pace/sessions/` |
