...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`pace webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `pace webhook subscribe`) |
...
pace cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/pace-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/pace-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
pace webhook subscribe github-pr-review \
  --events "pull_request" \
...
pace cron create "0 9 * * 1" \
  "Scan the NousResearch/pace-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/pace-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (pace_cli/commands.py, pace_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (pace_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
pace cron create "0 6 * * *" \
  "Run a dependency security audit on the pace-agent project.

1. cd ~/.pace/pace-agent && source .venv/bin/activate
...
```bash
pace webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
pace webhook subscribe alert-triage \
...
```python title="~/.pace/scripts/check-uptime.py"
...
pace cron create "every 30m" \
  ...
  --script ~/.pace/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
pace webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
pace cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the pace-agent codebase.
