...
```bash
pace teams-pipeline token-health
pace teams-pipeline token-health --force-refresh
```
...
```bash
pace teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
pace teams-pipeline maintain-subscriptions
pace teams-pipeline maintain-subscriptions --dry-run
```
...
Hermes ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.pace/scripts/`. First create the script:

```bash
mkdir -p ~/.pace/scripts
cat > ~/.pace/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec pace teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.pace/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
pace cron create "0 */12 * * *" \
...
```bash
pace cron list
pace cron status        # scheduler status
```
...
Type=oneshot
User=pace
EnvironmentFile=/etc/pace/env
ExecStart=/usr/local/bin/pace teams-pipeline maintain-subscriptions
```
...
```cron
0 */12 * * * /usr/local/bin/pace teams-pipeline maintain-subscriptions >> /var/log/pace/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.pace/.env` at the top of a wrapper script that crontab calls.
...
```bash
pace teams-pipeline subscriptions   # should show expirationDateTime advanced
pace teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
pace teams-pipeline list
pace teams-pipeline list --status failed
pace teams-pipeline show <job-id>
```

### Replay a stored job

```bash
pace teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
pace teams-pipeline fetch --meeting-id <meeting-id>
pace teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `pace teams-pipeline maintain-subscriptions --dry-run`
- inspect `pace teams-pipeline list --status failed`
...
- run `pace teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `pace teams-pipeline run`
...
- [ ] `pace teams-pipeline validate` returns an OK snapshot
- [ ] `pace teams-pipeline token-health --force-refresh` succeeds
