...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `pace send`."
---

# Pipe Script Output to Messaging Platforms

`pace send` is a small, scriptable CLI that pushes a message to any
...
- Piping any tool's output anywhere (`make | pace send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `pace
...
pace send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
pace send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
pace send --list

# Filter by platform
pace send --list telegram
```
...
`pace send` resolves the message body in this order:

1. **Positional argument** — `pace send --to telegram "hi"`
2. **`--file PATH`** — `pace send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | pace send --to telegram`
...
if [ "$ram_pct" -ge 85 ]; then
  pace send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `pace send` reuses your Hermes config, the same script works on
...
`pace send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  pace send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | pace send \
...
```bash
./train.py --epochs 200 && \
  pace send --to telegram "training done" || \
  pace send --to telegram "training failed (exit $?)"
```
...
pace send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(pace send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `pace send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `pace send` calls
the platform's REST endpoint directly using credentials from
`~/.pace/.env` and `~/.pace/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `pace gateway start` and retry.
...
The listing is built from `~/.pace/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`pace gateway
...
|----------|----------------|---------------------|---------------|----------|
| `pace send` | ✅ | ✅ | No (bot-token) | Everything below |
...
If you just need to pipe a raw string, reach for `pace send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `pace send` shares with cron delivery.
