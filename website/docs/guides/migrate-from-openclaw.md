...
`pace claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Hermes. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
:::tip
If your OpenClaw setup was multi-provider, `pace setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
pace claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.pace/` (by default a single restore-point archive is written before apply, under `~/.pace/backups/pre-migration-*.zip`; restorable with `pace import`). |
...
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.pace/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.pace/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.pace/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.pace/memories/MEMORY.md` | All daily files merged into main memory. |
...
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.pace/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.pace/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.pace/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.pace/skills/openclaw-imports/` |
...
These are saved to `~/.pace/migration/openclaw/<timestamp>/archive/` for manual review:
...
| Cron jobs | `archive/cron-config.json` | Recreate with `pace cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `pace webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `pace honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `pace skills config` |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Hermes manually via `pace config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.pace/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `pace status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — run `pace config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `pace whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `pace claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `pace config set`.

### Skills not appearing after migration

Imported skills land in `~/.pace/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `pace config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
