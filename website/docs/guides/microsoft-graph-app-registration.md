...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.pace/.env` at the end.
...
Put the three values you collected into `~/.pace/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.pace/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `pace gateway restart`.
