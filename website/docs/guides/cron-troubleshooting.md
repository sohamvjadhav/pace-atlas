...
If you're expecting jobs to fire automatically, you need a running gateway (`pace gateway` for foreground, or `pace gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `pace cron tick`.
...
```bash
date
pace cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.pace/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.pace/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.pace/.env` |
...
| `local` | Write access to `~/.pace/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `pace cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `pace skills install <skill-name>` or via `/skills` in the CLI.
...
2. `~/.pace/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `pace cron list`
...
```bash
ls ~/.pace/scripts/your-script.py   # Must exist
pace cron edit <job_id> --script ~/.pace/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `pace skills install <skill-name>`.
...
Jobs are stored in `~/.pace/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.pace/cron/jobs.json
chmod 600 ~/.pace/cron/jobs.json   # Your user should own it
```
...
```bash
pace cron list                    # Show all jobs, states, next_run times
pace cron run <job_id>            # Schedule for next tick (for testing)
pace cron edit <job_id>           # Fix configuration issues
pace logs                         # View recent Hermes logs
pace skills list                  # Verify installed skills
```
...
1. Run the job with `pace cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.pace/logs/agent.log` for scheduler messages and `~/.pace/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/pace-agent](https://github.com/NousResearch/pace-agent) with:
