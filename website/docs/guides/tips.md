...
Run `pace setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Nous Portal](/integrations/nous-portal).
:::
...
Forgot something from your last session? Run `pace -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `pace -r "my research project"`.
...
Want Hermes to have a stable default voice? Edit `~/.pace/SOUL.md` (or `$HERMES_HOME/SOUL.md` if you use a custom Hermes home). Hermes now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Hermes](/guides/use-soul-with-pace).
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `pace sessions list` and resume with `pace -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `pace pairing approve telegram XKGH5N7P` — simple and secure.
