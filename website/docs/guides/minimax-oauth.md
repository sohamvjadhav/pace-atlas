...
After the first login, credentials are stored under `~/.pace/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.pace/.env
```
...
```bash
pace auth add minimax-oauth --no-browser
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.pace/auth.json` under the `minimax-oauth` key.
...
```bash
pace config set model.default MiniMax-M2.7
pace config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.pace/config.yaml` will contain entries similar to:
...
```bash
pace --provider minimax-oauth    # canonical
pace --provider minimax-portal   # alias
pace --provider minimax-global   # alias
pace --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `pace setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
pace --provider minimax-oauth
```
...
**Fix:** run `pace auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Hermes raises a timeout error.

**Fix:** re-run `pace auth add minimax-oauth` (or `pace model`). The flow starts fresh.
...
If `pace` cannot open a browser window, use `--no-browser`:
...
**Fix:** run `pace model` and select MiniMax (OAuth), or run `pace auth add minimax-oauth`.
...
- [pace doctor](../reference/cli-commands.md)
