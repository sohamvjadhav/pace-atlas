...
**For non-NixOS users**, this only changes the install step. Everything after (`pace setup`, `pace gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage pace the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `pace`, `pace-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `pace setup` walks you through provider selection, `pace gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.pace/`.
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    pace-agent.url = "github:NousResearch/pace-agent";
  };

  outputs = { nixpkgs, pace-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        pace-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
That's it. `nixos-rebuild switch` creates the `pace` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o pace /dev/stdin /var/lib/pace/env
```

```nix
services.pace-agent.environmentFiles = [ "/var/lib/pace/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `pace` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `pace` in your shell creates a separate `~/.pace/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `pace` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `pace chat`, `pace sessions list`, `pace version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the pace codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.pace` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `pace` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo pace chat` manually.
:::
...
systemctl status pace-agent

# Watch logs (Ctrl+C to stop)
journalctl -u pace-agent -f

# If addToSystemPackages is true, test the CLI
pace version
pace config       # shows the generated config
```
...
```nix
services.pace-agent.configFile = /etc/pace/config.yaml;
```
...
| Give the agent a personality | `${services.pace-agent.stateDir}/.pace/SOUL.md` | manage the file directly |
...
| Override the pace package | `package` | `inputs.pace-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/pace"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart pace-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/pace.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."hermes-env" = { format = "yaml"; };
  };

  services.pace-agent.environmentFiles = [
...
```yaml
# secrets/pace.yaml (encrypted with sops)
hermes-env: |
...
{
  services.pace-agent = {
    authFile = config.sops.secrets."pace/auth.json".path;
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.pace-agent.stateDir}/.pace/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.

```nix
{
  services.pace-agent.documents = {
...
```nix
{
  services.pace-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.pace-agent.mcpServers.remote-api = {
...
```nix
{
  services.pace-agent.mcpServers.my-oauth-server = {
...
**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u pace` (native):

```bash
# Container mode
docker exec -it pace-agent \
  pace mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u pace HERMES_HOME=/var/lib/pace/.pace \
  pace mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
pace mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.pace/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/pace/.pace/mcp-tokens/
# Ensure: chown pace:pace, chmod 0600
```
...
```nix
{
  services.pace-agent.mcpServers.analysis = {
    command = "npx";
...
When pace runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `pace setup` | Config is declarative — edit `settings` in your Nix config |
| `pace config edit` | Config is generated from `settings` |
| `pace config set <key> <value>` | Config is generated from `settings` |
| `pace gateway install` | The systemd service is managed by NixOS |
| `pace gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it pace-agent pace config set ...` is also blocked)
...
When container mode is enabled, pace runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-pace-agent-0.1.0  ──►  /nix/store/... (ro)
~/.pace -> /var/lib/pace/.pace       (symlink bridge, per hostUsers)
/var/lib/pace/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/pace gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/pace` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart pace-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the pace package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/pace` is preserved (these are bind mounts).
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current pace package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `pace plugins install` needed.
...
```nix
services.pace-agent.extraPlugins = [
...
For pip-packaged plugins that register via `[project.entry-points."hermes_agent.plugins"]` (e.g., [rtk-pace](https://github.com/ogallotti/rtk-pace)):

```nix
services.pace-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the pace wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in pace-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.pace-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.pace-agent.url = "github:NousResearch/pace-agent";
  outputs = { pace-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ pace-agent.overlays.default ];
    # Then:
    #   pkgs.pace-agent.override { extraPythonPackages = [...]; }
    #   pkgs.pace-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.pace-agent.settings.plugins.enabled = [
  "hermes-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core pace dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `pace` and `pace-agent` binaries exist and `pace version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `pace --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true pace config set ...` prints the NixOS error |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the pace-agent service |
| `package` | `package` | `pace-agent` | The pace-agent package to use |
| `user` | `str` | `"pace"` | System user |
| `group` | `str` | `"pace"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/pace"` | State directory (`HERMES_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `pace` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `pace gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the pace user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.pace` symlink to the service stateDir and are auto-added to the `pace` group |
...
```
/var/lib/pace/                     # stateDir (owned by pace:pace, 0750)
├── .pace/                         # HERMES_HOME
...
| `/data` | `/var/lib/pace` | `rw` | All state, config, workspace |
| `/home/pace` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update pace-agent
...
```bash
systemctl status pace-agent
docker ps -a --filter name=pace-agent
docker inspect pace-agent --format='{{.State.Status}}'
docker exec -it pace-agent bash
docker exec pace-agent readlink /data/current-package
docker exec pace-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop pace-agent
docker rm -f pace-agent
sudo rm /var/lib/pace/.container-identity
sudo systemctl start pace-agent
```
...
```bash
# Native mode
sudo -u pace cat /var/lib/pace/.pace/.env

# Container mode
docker exec pace-agent cat /data/.pace/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec pace-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u pace-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `pace version` shows old version | Container not restarted | `systemctl restart pace-agent` |
| Permission denied on `/var/lib/pace` | State dir is `0750 pace:pace` | Use `docker exec` or `sudo -u pace` |
| `nix-collect-garbage` removed pace | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "pace-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user pace` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart pace-agent` |
