...
The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `pace` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
The root-mode **FHS layout** (`/usr/local/lib/…`, `/usr/local/bin/pace`) matches where other system-wide developer tools land on Linux. It's useful for shared-machine deployments where one system install should serve every user. Per-user config (auth, skills, sessions) still lives under each user's `~/.pace/` or explicit `HERMES_HOME`.
...
3. **Make `pace` available to the service user's shells.** The installer writes the launcher to `~/.local/bin/pace`. System service accounts often have a minimal PATH that doesn't include `~/.local/bin`. Either add it to the user's environment, or symlink the launcher into a system location:
   ```bash
   # Option A — add to the service user's profile
   echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc

   # Option B — symlink system-wide (run as an admin)
   sudo ln -s /home/pace/.pace/pace-agent/venv/bin/pace /usr/local/bin/pace
   ```

4. **Verify:** `pace doctor` should now run cleanly. If you get `ModuleNotFoundError: No module named 'dotenv'`, you're invoking the repo source `pace` file (`~/.pace/pace-agent/pace`) with system Python instead of the venv launcher (`~/.pace/pace-agent/venv/bin/pace`) — fix step 3.
...
| Problem | Solution |
|---------|----------|
| `pace: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `pace model` to configure your provider, or `pace config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `pace config check` then `pace config migrate` |

For more diagnostics, run `pace doctor` — it will tell you exactly what's missing and how to fix it.
