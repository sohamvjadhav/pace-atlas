...
:::tip
`pace update` automatically detects new configuration options and prompts you to add them. If you skipped that prompt, you can manually run `pace config check` to see missing options, then `pace config migrate` to interactively add them.
:::

### What happens during an update

When you run `pace update`, the following steps occur:
...
By default `pace update` tracks `origin/main`. Pass `--branch <name>` to update against a different branch — useful for QA channels, feature branches, or release-candidate testing:

```bash
pace update --branch release-candidate
pace update --check --branch experimental   # preview behindness only
```
...
### Windows: another `pace.exe` is running

On Windows, `pace update` will refuse to run if it detects another `pace.exe` process holding the venv's entry-point executable open — most commonly the Hermes Desktop app's spawned backend, an open `pace` REPL in another terminal, or a running gateway:

```
$ pace update
✗ Another pace.exe is running:
    PID 12345  pace.exe

  Updating now would fail to overwrite ...\venv\Scripts\pace.exe because
  Windows blocks REPLACE on a running executable.

  Close Hermes Desktop, exit any open `pace` REPLs, and
  stop the gateway (`pace gateway stop`) before retrying.
  Override with `pace update --force` if you've already
...
`pace update` handles the main update path, but a quick validation confirms everything landed cleanly:

1. `git status --short` — if the tree is unexpectedly dirty, inspect before continuing
2. `pace doctor` — checks config, dependencies, and service health
3. `pace --version` — confirm the version bumped as expected
4. If you use the gateway: `pace gateway status`
5. If `doctor` reports npm audit issues: run `npm audit fix` in the flagged directory

:::warning Dirty working tree after update
If `git status --short` shows unexpected changes after `pace update`, stop and inspect them before continuing. This usually means local modifications were reapplied on top of the updated code, or a dependency step refreshed lockfiles.
:::

### If your terminal disconnects mid-update

`pace update` protects itself against accidental terminal loss:

- The update ignores `SIGHUP`, so closing your SSH session or terminal window no longer kills it mid-install. `pip` and `git` child processes inherit this protection, so the Python environment cannot be left half-installed by a dropped connection.
- All output is mirrored to `~/.pace/logs/update.log` while the update runs. If your terminal disappears, reconnect and inspect the log to see whether the update finished and whether the gateway restart succeeded:

```bash
tail -f ~/.pace/logs/update.log
```

- `Ctrl-C` (SIGINT) and system shutdown (SIGTERM) are still honored — those are deliberate cancellations, not accidents.

You no longer need to wrap `pace update` in `screen` or `tmux` to survive a terminal drop.
...
Compare against the latest release at the [GitHub releases page](https://github.com/NousResearch/pace-agent/releases).
...
pace config check
pace config migrate   # Interactively add any missing options
```
...
:::warning
Rolling back may cause config incompatibilities if new options were added. Run `pace config check` after rolling back and remove any unrecognized options from `config.yaml` if you encounter errors.
:::
...
nix flake update pace-agent

# Or rebuild with the latest
nix profile upgrade pace-agent
```
...
The uninstaller gives you the option to keep your configuration files (`~/.pace/`) for a future reinstall.

### Manual Uninstall

```bash
rm -f ~/.local/bin/pace
rm -rf /path/to/pace-agent
rm -rf ~/.pace            # Optional — keep if you plan to reinstall
```
...
# macOS: launchctl remove ai.pace.gateway
```
:::
