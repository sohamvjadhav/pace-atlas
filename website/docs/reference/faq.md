...
Set your provider with `pace model` or by editing `~/.pace/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
- [Use MCP with Hermes](../guides/use-mcp-with-pace.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Hermes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.pace/`.

### Can I use it offline / with local models?

Yes. Run `pace model`, select **Custom endpoint**, and enter your server's URL:
...
#### `pace: command not found` after installation
...
**Solution:** Hermes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.pace/config.yaml`:
...
**Solution:** Exit your session and use `pace model` from your terminal to add new providers:
...
After adding a new provider via `pace model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `pace model` (from terminal) |
| Enter/change API keys | `pace model` (from terminal) |
...
# Or set directly
pace config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.pace/.env` for conflicting entries.
:::
...
# Set a valid model
pace config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
pace chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `pace chat --provider <alternative>` to route to a different backend
...
pace chat --model openrouter/google/gemini-3-flash-preview
```
...
- Or switch to the terminal interface for administrative tasks: `pace chat`
...
cat ~/.pace/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `pace gateway setup`
- Check gateway logs: `cat ~/.pace/logs/gateway.log | tail -50`
...
Configure in `~/.pace/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
#### WSL: Gateway keeps disconnecting or `pace gateway start` fails
...
tmux new -s pace 'pace gateway run'
# Reattach later: tmux attach -t pace

# Option 3: Background via nohup
nohup pace gateway run > ~/.pace/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'pace gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `pace gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
pace gateway install    # Re-snapshots your current PATH
pace gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.pace.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `pace chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `pace chat -t "terminal"`
...
cd ~/.pace/pace-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.pace/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
pace config show | grep -A 12 mcp_servers
...
- [Use MCP with Hermes](/guides/use-mcp-with-pace)
...
### What happens when I run `pace update`?

`pace update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `pace update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.pace/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Hermes can route subagents to a different model automatically. Set this in `~/.pace/config.yaml`:
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `pace skills config` settings don't seem to take effect.

**Solution:** Use `pace skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`pace gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
2. On the **source machine**, create a full backup:
   ...
   pace backup
   ```
   This creates a zip of your entire `~/.pace/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   pace import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `pace setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
pace profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
pace profile import ./work-backup.tar.gz work
```
...
### `pace backup` vs `pace profile export`

| Feature | `pace backup` | `pace profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.pace` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='pace-agent' ~/.pace/ newmachine:~/.pace/
```

:::tip
`pace backup` produces a consistent snapshot even while Hermes is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
pace config show | head -20

# Re-run model selection
pace model

# Or test with a known-good model
pace chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/pace-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Hermes version (`pace --version`), and the full error message
