...
Hermes fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `pace-agent` release.
...
```
https://pace-agent.nousresearch.com/docs/api/model-catalog.json
```
...
| When | What happens |
|---|---|
| `/model` or `pace model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.pace/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in pace_cli/models.py).
