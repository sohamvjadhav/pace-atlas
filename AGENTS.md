# PACE Atlas - Development Guide

Instructions for AI coding assistants and developers working on the pace-agent codebase.
...
`$HOME/.pace/pace-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── pace_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
├── ui-tui/               # Ink (React) terminal UI — `pace --tui`
...
**User config:** `~/.pace/config.yaml` (settings), `~/.pace/.env` (API keys only).
**Logs:** `~/.pace/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `pace logs [--follow] [--level ...] [--session ...]`.
...
- **Skin engine** (`pace_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.pace/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`pace_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/pace` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `pace_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `pace --tui` or `PACE_TUI=1`.
...
### TUI in the Dashboard (`pace dashboard` → `/chat`)

The dashboard embeds the real `pace --tui` — **not** a rewrite.  See `pace_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `pace_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `pace --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `pace --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_hermes_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `PACE_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".pace"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `pace_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `pace_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `pace tools`, `pace setup`, most CLI subcommands | `pace_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`pace_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
pace_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.pace/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `pace_cli/skin_engine.py`:
...
Users create `~/.pace/skins/<name>.yaml`:
...
`~/.pace/plugins/` and pip-installed entry points.

### General plugins (`pace_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.pace/plugins/`, `./.pace/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `pace` at startup so
  `pace <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `pace <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `pace --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `pace_cli/main.py`, etc.).
...
into `~/.pace/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `pace memory setup` / `post_setup()` without
...
2. User: `$PACE_HOME/plugins/model-providers/<name>/`
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `pace skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.pace.tags`, `metadata.pace.category`,
`metadata.pace.related_skills`, `metadata.pace.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.pace.*` by the loader.
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "PACE Atlas" is the secondary collaborator. If the
   contributor's commit shows "PACE Atlas" as author (because they
...
lives in the `pace-agent-dev` skill at
...
Enable/disable per platform via `pace tools` (the curses UI) or the
...
go to `~/.pace/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `pace_cli/curator.py` wires `pace curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.pace/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `pace cron <verb>`
...
- File lock at `~/.pace/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `pace kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `pace_cli/kanban.py` wires `pace kanban` with verbs
...
`PACE_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `pace_cli/main.py` sets
`PACE_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all PACE_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.pace` or `Path.home() / ".pace"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".pace" / "config.yaml"
   ```

2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.pace` for default or `~/.pace/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.pace/config.yaml")
   ```
...
4. **Tests that mock `Path.home()` must also set `PACE_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".pace"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"PACE_HOME": str(tmp_path / ".pace")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not PACE_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".pace" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
   This is intentional — it lets `pace -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.pace` paths
Use `get_hermes_home()` from `hermes_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.pace` breaks profiles — each profile
has its own `PACE_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `pace_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `pace_cli/curses_ui.py` — see
`pace_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `PACE_HOME`.

### Tests must not write to `~/.pace/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `PACE_HOME` to a temp dir. Never hardcode `~/.pace/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_hermes_home()` resolve within the temp dir.
Use the pattern from `tests/pace_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PACE_HOME", str(home))
    return home
```
