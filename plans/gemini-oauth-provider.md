...
## Token Lifecycle
- Store at `~/.pace/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `pace_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `pace_cli/models.py` — Add Gemini model catalog
4. `pace_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `pace_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `pace_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- pace-agent Copilot OAuth: `pace_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
