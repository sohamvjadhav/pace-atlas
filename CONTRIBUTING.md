# Contributing to PACE Atlas

Thank you for contributing to PACE Atlas! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `pace skills browse` (labeled "official") and install it with `pace skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `pace skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.pace/plugins/` (or via a pip entry point).
...
- Integrate with `pace memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.pace/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.pace/config.yaml
touch ~/.pace/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.pace/.env
```
...
│
├── pace_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.pace/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (pace-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.pace/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.pace/`)

| Path | Purpose |
|------|---------|
| `~/.pace/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.pace/.env` | API keys and secrets |
| `~/.pace/auth.json` | OAuth credentials (Nous Portal) |
| `~/.pace/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.pace/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.pace/state.db` | SQLite session database |
| `~/.pace/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.pace/cron/` | Scheduled job data |
| `~/.pace/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.pace/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Four fields are supported under `metadata.pace`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `pace setup` or update `~/.pace/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "PACE Atlas" is the secondary collaborator. If the contributor's commit shows "PACE Atlas" as author because they used Hermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **Test it.** Run `pace --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `pace_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `pace_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the pace wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `pace_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `pace_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `pace_cli/gateway_windows.py::
...
2. **Test manually**: Run `pace` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/sohamvjadhav/pace-atlas/issues)
- Include: OS, Python version, Hermes version (`pace version`), full error traceback
